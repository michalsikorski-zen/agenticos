...
- **Security headers** — the frontend serves a full Content-Security-Policy (`default-src 'self'`, a `connect-src` of `'self'` plus `ws:`/`wss:` for the chat WebSocket, `object-src 'none'`, `base-uri 'self'`, `frame-ancestors 'none'`) plus `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin` and a `Permissions-Policy` that denies camera/geolocation and allows the microphone only for speech-to-text. The policy lives in `frontend/src/lib/csp.ts` and the headers in `frontend/src/lib/security-headers.ts`, both asserted by tests; see [Deployment](docs/deployment.md#security-headers). A reverse proxy passes these through rather than adding its own — the bundled `nginx/nginx.conf` sets only `Strict-Transport-Security`.
